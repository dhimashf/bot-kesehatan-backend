...
def register_user(form_data: WebAccountCreate):
    """
    ...
    new_user = user_service.create_user(user=form_data)
    if not new_user:
...
def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    user = web_auth_service.authenticate_user(email=form_data.username, password=form_data.password)
...
def set_password(request: SetPasswordRequest):
    ...
    """
    success = web_auth_service.set_user_password(email=request.email, password=request.password)
    if not success:
